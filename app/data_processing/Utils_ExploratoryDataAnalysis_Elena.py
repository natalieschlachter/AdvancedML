...
def read_csv_data(csv_file_path):
    ...
    """
    import pandas as pd
    csv_data = pd.read_csv(csv_file_path, header=None)  # Read CSV without headers
    return csv_data.iloc[:, 0].tolist()  # Return the first column as a list
...
def DBSCAN_clustering(folder_path, signal, window_size=20, eps=0.5, min_samples=15):
    """
    Perform density clustering on a time series and plot the results.

    Parameters:
    - folder_path: str
        Path to the folder containing time series CSV files.
    - signal: str
        Type of signal ('bvp' or 'eda').
    - window_size: int, default=20
        The size of the sliding window used for segmenting the time series.
    - eps: float, default=0.5
        The maximum distance for DBSCAN to consider points as part of the same cluster.
    - min_samples: int, default=5
        The number of samples in a neighborhood for DBSCAN.

    Returns:
    - labels: array, shape (n_samples,)
        Cluster labels for the time series data.
    """
    if signal == "bvp":
        print(f"BVP DBSCAN")
        files = [f for f in os.listdir(folder_path) if f.endswith('.csv') and f.startswith('bvp')]
    ...
    fig.suptitle(f"DBSCAN of {signal.upper()} Segments", fontsize=16, fontweight='bold')

    for i, file in enumerate(files):
        if i >= 3:
            break  # Only plot the first 3 files
        
        file_path = os.path.join(folder_path, file)
        time_series = read_csv_data(file_path)
              
        # Step 1: Sliding Window Segmentation
        n_samples = len(time_series)
        n_segments = n_samples - window_size + 1
        segments = []
        
        for j in range(n_segments):
            segment = time_series[j:j + window_size]
            segments.append(segment)
        
        segments = np.array(segments)
    
        # Step 2: Feature Extraction (mean, variance, skewness, kurtosis)
    
        features = []
        for segment in segments:
            mean = np.mean(segment)
            variance = np.var(segment)
            if variance < 1e-6:  # If variance is very small, skip skewness and kurtosis
                features.append([mean, variance, 0, 0])
            else:
                features.append([mean, variance, skew(segment), kurtosis(segment)])
        ...
        # Step 4: Density Clustering (DBSCAN)
        dbscan = DBSCAN(eps=eps, min_samples=min_samples)
        labels = dbscan.fit_predict(features_scaled)
        
        # Step 5: Plotting the Clustering Results
        sns.scatterplot(
            x=features_scaled[:, 0], y=features_scaled[:, 1], hue=labels, 
            palette='deep', s=30, alpha=0.6, ax=axes[i]
        )
        axes[i].set_xlabel('Feature 1 (Standardized)', fontsize=12)
        axes[i].set_ylabel('Feature 2 (Standardized)', fontsize=12)
        axes[i].set_title(f"Plot for {file}", fontsize=14)
        axes[i].grid(True, linestyle='--', alpha=0.6)
        axes[i].legend_.set_title('Cluster Label')
    
    plt.tight_layout(rect=[0, 0, 1, 0.95], w_pad=5.0)
    plt.show()
